...
const UserProfilesDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    ...
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.position?.name}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="name" header="User Name" body={pTemplate0} sortable style={{ minWidth: "8rem" }} />
            <Column field="position" header="position" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
