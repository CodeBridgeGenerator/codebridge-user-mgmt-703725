...
const RefPositionsFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [reportingToidIds, setreportingToidIds] = useState();
...
client
            ...
            .find({ query: { $select: ["_id","name"] } })
            .then((data) => {
              setreportingToidIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = refPositionsFakerFactory(fakerCount,roleIds,reportingToidIds);
