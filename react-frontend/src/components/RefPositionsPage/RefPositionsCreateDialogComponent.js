...
const RefPositionsCreateDialogComponent = (props) => {
    ...
    const reportingToidOptions = reportingToid.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create ~cb-service-displayName~" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div className="col-12 md:col-6 field mt-5">
                <span className="p-float-label">
                    <Dropdown id="role" value={_entity?.role} optionLabel="name" optionValue="value" options={roleOptions} onChange={(e) => setValByKey("role", {_id : e.value})} />
            ...
            <div className="col-12 md:col-6 field mt-5">
                <span className="p-float-label">
                    <Dropdown id="reportingToid" value={_entity?.reportingToid} optionLabel="name" optionValue="value" options={reportingToidOptions} onChange={(e) => setValByKey("reportingToid", {_id : e.value})} />
                    <label htmlFor="reportingToid">reportingToid:</label>
                </span>
            </div>
            <div className="col-12 md:col-6 field mt-5">
                <span className="p-float-label">
